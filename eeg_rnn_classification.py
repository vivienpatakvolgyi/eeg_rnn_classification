...
if results: 
    ...
    if 'a)' in train_test:
        ...
        st.write('In this case, you can upload the fourth user\'s data in csv. After downloading the original .csv file, please drag and drop here to start the prediction and see the results')
        ...
        st.write('Sadly, in this case we don\'t have the data in csv, because it was separated during the data processing. However, I\'m gonna show you the processed data structure right before we use it for prediction. In option a), after data processing, we have the same exact structure but with different data. This data is only the 20\% of the full dataset and it was selected randomly from all user\'s time series data as sequences. In option a), we used 25% of the full dataset for prediction and it\'s from an entirely different user than the ones we used for training.')
